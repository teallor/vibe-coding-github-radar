...
on:
  schedule:
    # 当前：每天 UTC 23:42（北京时间次日 07:42）
    - cron: '42 23 * * *'
```

### 为什么不是直接设置 08:00？

最近一次完整真实运行从触发到飞书发送成功耗时 **13 分 58 秒**：初始化约 12 秒、GitHub Radar 约 5 分 25 秒、播客约 4 分 27 秒、AI/Codex 生态约 3 分 53 秒、飞书发送约 1 秒。因此工作流在北京时间 **07:42** 启动，正常约 **07:56** 完成；考虑 GitHub 排队及 Gemini 波动，目标到达窗口为 **07:56–08:01**。GitHub 官方说明整点更容易拥堵，所以采用 42 分启动而不是整点。
...
- 三类雷达每天北京时间 07:42 在同一个 workflow 中启动，目标约 08:00 完成飞书推送
